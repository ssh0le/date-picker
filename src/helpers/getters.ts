...
export const getCalendarByYear = (
  date: Date,
  minDate?: Date,
  maxDate?: Date,
): Date[] => {
  ...
  const [minYear, minMonth] = minDate
    ? getDestructuredDate(minDate)
    : [year, 0];
  const [maxYear, maxMonth] = maxDate
    ? getDestructuredDate(maxDate)
    : [year, 11];
...
export const getDayOfWeekIndex = (
  date: Date,
